...
class RegionalInductorInvokeSubgraphTests(torch._inductor.test_case.TestCase):
    ...
    def test_repeated_blocks(self, serialize):
        ...
        opt_mod = torch.compile(
            mod,
            backend=aot_eager_regional_inductor(
                serialize=serialize, on_invoke_subgrah=True, captured_gms=captured_gms
            ),
            fullgraph=True,
        )
        x = torch.randn(10, requires_grad=True)
...
def forward(self, arg0_1, arg1_1):
    ...
    def test_flex_attention(self, serialize):
        ...
        opt_fn = torch.compile(
            fn,
            backend=aot_eager_regional_inductor(serialize, on_invoke_subgrah=True),
            fullgraph=True,
        )
        ...
        with capture_invoke_subgraph_inductor_compile_gms() as captured_gms:
            ...
            # TODO: the returned values will be detached twice (once in subgraph, once outside), will this hurt performance?
...
def forward(self, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8, detach_2, detach_3, tangents_0):
    ...
    # TODO: test compiling two different subgraphs
