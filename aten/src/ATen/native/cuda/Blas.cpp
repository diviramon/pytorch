...
bool launchGemmAndEpilogueCublasLt(
  ...
  int64_t pre_activation_ld = (opt_pre_activation.has_value()
      ? (args.result->is_contiguous()
        // Contiguous result implies transposed problem
        // which turns it into row-major
        ? opt_pre_activation.value().stride(-1)
        // Otherwise result is already row-major and
        // there is no need in transposing the problem
